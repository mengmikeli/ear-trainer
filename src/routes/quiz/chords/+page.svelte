<script lang="ts">
	...
	function selectAnswer(choice: { id: string; name: string }) {
		...
		const correct = choice.id === question.chord.id;
		...
		// Update chord state (flat fields)
		const s = state.chords[question.chord.id];
		s.attempts++;
		if (correct) {
			s.correct++;
			s.streak++;
		...
		// Per-voicing stat recording
		const voicing = question.voicing;
		const voicingStats = s.voicings[voicing];
		voicingStats.attempts++;
		if (correct) {
			voicingStats.correct++;
			voicingStats.streak++;
		} else {
			voicingStats.streak = 0;
		}
		voicingStats.lastSeen = Date.now();

		const sm2v = calculateSm2(voicingStats.easeFactor, quality);
		voicingStats.easeFactor = sm2v.easeFactor;
		voicingStats.nextReview = Date.now() + sm2v.intervalMs;
	...
	const voicingBreakdown = $derived(() => {
		const voicings: Record<string, { total: number; correct: number }> = {};
		for (const r of results) {
			if (!voicings[r.voicing]) voicings[r.voicing] = { total: 0, correct: 0 };
			voicings[r.voicing].total++;
			if (r.correct) voicings[r.voicing].correct++;
		}
		return voicings;
	});
	...
	function replayMissed(r: QuestionResult) {
		...
		setTimeout(() => { replayingIndex = null; }, 1500);
	}
</script>
...
<div class="summary">
	...
	{#if Object.keys(voicingBreakdown()).length > 1}
		<div class="section-label">PER VOICING</div>
		<div class="mode-rows">
			{#each Object.entries(voicingBreakdown()) as [voicing, stats]}
				<div class="mode-row">
					<span class="mode-glyph">{voicingLabel[voicing] ?? voicing}</span>
			...
			{#each wrongAnswers as r, i}
				<button class="missed-card" class:replaying={replayingIndex === i} onclick={() => replayMissed(r)}>
					...
					<div class="missed-card-content">
						...
						<span class="missed-mode">{voicingLabel[r.voicing]}</span>
					</div>
				</button>
			{/each}
		</div>
...
<div class="quiz">
	...
	<div class="top">
		...
		<div class="top-controls">
			...
			<span class="mode-icon">{question ? voicingLabel[question.voicing] : ''}</span>
			<div class="top-right">
				<button class="arp-toggle" class:active={isArpeggiated} onclick={toggleArp}>
					{isArpeggiated ? 'ARP' : 'BLK'}
				</button>
...
<style>
	...
	/* Summary screen — same as intervals */
	.summary {
	...
	.mode-glyph {
		font-size: 0.4rem; font-weight: 900; color: var(--marathon-blue);
		width: 2.5rem; text-align: center; letter-spacing: 0.05em;
	}
	.mode-stat {
